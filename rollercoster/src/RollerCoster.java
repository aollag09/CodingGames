 ...
 * statement. The attraction contains a limited number L of places. The attraction can only function
 * C number of times per day. The queue contains a number N of groups. Each group contains a number
 * Pi of people.
 *
 * <p>L : 1.0E7 C : 9000000.0 N : 1000 PI : 71670
 */
class Solution {
  ...
  public static void main(String args[]) {
    ...
    while (round < C) {
      ...
      if (oldcursor == cursor) {
        ...
        for (int i = oldcursor; i <= cursor; i++) people += queue[i];
      } else {
        for (int i = oldcursor; i < N; i++) people += queue[i];
        for (int i = 0; i <= cursor; i++) people += queue[i];
      }
